<div align="center">
  ...
  <p>流式对话 · 智能分析 · RAG 检索增强 · Skills 扩展 · 多用户会话管理</p>
...
当前前端产品形态为 **小维助手**，同时提供独立的 `智能分析` 工作流入口，使项目既能满足自然语言交互，也能承担更偏结构化的运维分析任务。
...
| 智能分析 工作流 | 支持 `/api/ai_ops` 流式分析过程与运维报告输出 |
...
页面右下角的 `智能分析` 按钮用于触发独立的智能运维分析链路。接口支持流式输出，可逐步展示分析过程和最终报告，并将结果并入当前会话上下文，便于后续继续追问。
...
- 前端层：负责登录注册、对话交互、智能分析 展示、近期会话管理
...
| 接口 | 说明 |
| --- | --- |
| `POST /api/ai_ops` | 智能分析 流式分析与报告输出 |
